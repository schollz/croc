...
function initialize() {
  ...
  ready = (async () => {
    ...
    for (
      let attempts = 0;
      !self.crocTunnelWasm && attempts < 100;
      attempts += 1
    ) {
      await new Promise((resolve) => setTimeout(resolve, 10));
    }
    if (!self.crocTunnelWasm)
      throw new Error("croc Tunnel WASM did not initialize");
  })();
  return ready;
}
...
self.addEventListener("message", async (event) => {
  ...
  try {
    await initialize();
    const fn = self.crocTunnelWasm[method];
    if (typeof fn !== "function")
      throw new Error(`Unknown Tunnel WASM method: ${method}`);
