...
func TestGenerateRandomPin(t *testing.T) {
	// GenerateRandomPin must be able to emit every decimal digit 0-9 so
	// the numeric prefix contributes its full 10^NbPinNumbers of entropy.
	// Each position is drawn uniformly from [0,10), so the probability that
	// any digit is absent after this many samples is astronomically small,
	// making this effectively deterministic.
	seen := make(map[rune]bool)
	for i := 0; i < 10000; i++ {
		pin := GenerateRandomPin()
		assert.Len(t, pin, NbPinNumbers)
		for _, r := range pin {
			assert.True(t, r >= '0' && r <= '9', "pin contains non-digit: %q", pin)
			seen[r] = true
		}
	}
	for d := '0'; d <= '9'; d++ {
		assert.Truef(t, seen[d], "digit %q was never generated by GenerateRandomPin", string(d))
	}
}
